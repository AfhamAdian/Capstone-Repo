...
describe('survey-token', () => {
  it('round-trips a valid payload through encode/decode', () => {
    const payload = { surveyId: 42, cycleId: 'manual-abc', deadline: new Date(Date.now() + 60_000).toISOString() };
  ...
  it('produces different ciphertext for the same payload (random IV)', () => {
    const payload = { surveyId: 1, cycleId: 'auto-1-2026-07-r1', deadline: new Date().toISOString() };
    expect(encodeToken(payload)).not.toBe(encodeToken(payload));
  });

  it('rejects a token with a flipped byte instead of throwing', () => {
    const token = encodeToken({ surveyId: 7, cycleId: 'c', deadline: new Date().toISOString() });
  ...
  it('flags a past deadline as expired and a future one as not', () => {
    const past = { surveyId: 1, cycleId: 'c', deadline: new Date(Date.now() - 1000).toISOString() };
    const future = { surveyId: 1, cycleId: 'c', deadline: new Date(Date.now() + 1000).toISOString() };
