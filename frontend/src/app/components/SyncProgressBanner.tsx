...
function ToolStatusIcon({ status }: { status: SyncToolUiStatus }) {
  if (status === "completed") return <Check size={13} className="text-emerald-500" />;
  if (status === "failed") return <X size={13} className="text-red-500" />;
  if (status === "syncing") return <RefreshCw size={13} className="animate-spin text-primary" />;
    ...
    <div className={`mr-auto w-full max-w-md border bg-card px-4 py-3 ${tone} ${className}`} role="status" aria-live="polite" aria-atomic="false">
      <div className="flex items-start gap-2.5">
        {snapshot.status === "failed" ? (
          ...
          <RefreshCw size={15} className={`mt-0.5 shrink-0 text-primary ${active ? "animate-spin" : ""}`} />
        )}
          ...
          {snapshot.tools.map((item) => (
            <div
              ...
              className={`flex w-full items-center gap-2 border bg-background px-2.5 py-2 text-sm ${
                item.status === "failed" ? "border-red-500/30" : item.status === "completed" ? "border-emerald-500/30" : item.status === "syncing" ? "border-primary/30" : "border-border"
              }`}
            >
