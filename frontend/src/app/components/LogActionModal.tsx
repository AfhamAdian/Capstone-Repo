  ...
  return (
    <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
      ...
      <motion.div initial={{scale:0.97,opacity:0,y:8}} animate={{scale:1,opacity:1,y:0}} exit={{scale:0.97,opacity:0,y:8}} transition={{duration:0.16}}
        onClick={e=>e.stopPropagation()} className="w-full max-w-4xl bg-card border border-border shadow-overlay flex max-h-[90vh]">
        ...
        <div className="flex-1 flex flex-col min-w-0">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
              ...
              <div>
                ...
                <div className="relative" ref={dRef}>
                  <button onClick={()=>setDropOpen(!dropOpen)}
                    className={`w-full flex items-center justify-between bg-input-background border px-4 py-3 text-base text-left transition-colors ${dropOpen?"border-primary":"border-border hover:border-primary/50"}`}>
                  ...
                  <AnimatePresence>
                    {dropOpen&&(
                      <motion.div initial={{opacity:0,y:-4}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-4}} transition={{duration:0.12}}
                        className="absolute left-0 right-0 top-full mt-1 bg-popover border border-border shadow-overlay z-10 max-h-64 overflow-y-auto">
                        ...
                        {projects.map(p=>(
                          <button key={p.id} onClick={()=>toggle(projectValue(p))}
                            ...
                            <div className="flex-1 text-left">
                              <div className={`text-base font-semibold ${sel.includes(projectValue(p))?"text-link":"text-foreground"}`}>{p.name}</div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-input-background border border-border px-4 py-3 text-base placeholder:text-muted-foreground focus:border-primary resize-none transition-colors leading-relaxed"
                />
              ...
              <div>
                ...
                <textarea value={reason} onChange={e=>setReason(e.target.value)} rows={3}
                  ...
                  className="w-full bg-input-background border border-border px-4 py-3 text-base placeholder:text-muted-foreground focus:border-primary resize-none transition-colors"/>
              </div>
              ...
              <div>
                ...
                <input type="date" value={date} onChange={e=>setDate(e.target.value)}
                  className="bg-input-background border border-border px-4 py-3 text-base focus:border-primary transition-colors" style={{fontFamily:"var(--font-mono)"}}/>
              </div>
            </div>
          )}
          ...
          {!submitted&&(
            <div className="px-6 py-4 border-t border-border">
              {error&&<div className="mb-3 flex items-center gap-2 border border-destructive/40 bg-destructive/10 px-3 py-2 text-sm text-destructive"><AlertCircle size={13}/>{error}</div>}
              <div className="flex items-center justify-between">
              <button onClick={onClose} className="text-base text-muted-foreground hover:text-foreground transition-colors">Cancel</button>
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {searching?(
              <div className="text-sm text-muted-foreground text-center pt-8 leading-relaxed px-2">
                <RefreshCw size={18} className="animate-spin mx-auto mb-3 text-link"/>Comparing with past actions…
              </div>
            ):searchError?(
              <div className="text-sm text-attention text-center pt-8 leading-relaxed px-2">
                ...
                {visibleSimilar.map((action,idx)=>(
                  <div key={action.id} className="border border-border bg-card p-3.5">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div className="text-sm font-semibold text-foreground leading-snug">{action.problem}</div>
                      <span className="text-xs text-muted-foreground shrink-0 mt-0.5 font-mono">#{idx+1}</span>
                    </div>
                    {actionSimilarityLabel(action.similarity)&&<div className="text-xs font-semibold text-link mb-2">{actionSimilarityLabel(action.similarity)}</div>}
                    <div className="text-xs text-muted-foreground mb-2.5 leading-relaxed">{action.actionTaken}</div>
                    <div className="flex items-center justify-between">
                      <div className="flex gap-0.5">
                        {action.effectiveness!==null
                          ?Array.from({length:5}).map((_,i)=><Star key={i} size={10} className={i<action.effectiveness!?"text-attention fill-attention":"text-muted-foreground"}/>)
