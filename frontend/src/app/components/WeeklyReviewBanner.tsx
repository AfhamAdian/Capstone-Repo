...
export function WeeklyReviewBanner({queue,userId,onReview}:{queue:ActionReviewQueue;userId:number;onReview:()=>void}) {
  ...
  const cohort=[queue.fromLastWeek.length?`${queue.fromLastWeek.length} from last week`:"",queue.earlier.length?`${queue.earlier.length} from earlier`:""].filter(Boolean).join(" · ");
  ...
  return <div role="region" aria-label="Actions ready for effectiveness review" className="shrink-0 border-b border-border bg-card px-6 py-3">
    <div className="max-w-6xl mx-auto border border-border border-l-[3px] border-l-amber-400 bg-muted/20 px-4 py-3 flex items-center gap-4 flex-wrap">
      <div className="w-8 h-8 bg-amber-400/15 flex items-center justify-center text-amber-500 shrink-0"><Star size={16} className="fill-amber-400"/></div>
      ...
      <button onClick={onReview} className="text-sm font-semibold text-primary flex items-center gap-1.5 whitespace-nowrap hover:opacity-75">Review actions <ArrowRight size={13}/></button>
